...
# ==========================================
# MOTOR DE DETECCIÓN NÚCLEO-FÍSICO (VERSIÓN ORO)
# ==========================================
...
def procesar_campo_visual(image_bytes):
    try:
        ...
        # --- B. ESCALA CORREGIDA (Línea del Eje - 4% de margen) ---
        # Aislamos las líneas para medir el eje sin chocar con texto
        kernel_h_escala = cv2.getStructuringElement(cv2.MORPH_RECT, (int(ancho*0.1), 1))
        lineas_h_pura = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel_h_escala)
        
        fila_eje_h = lineas_h_pura[cy-5 : cy+5, cx:]
        _, x_h = np.where(fila_eje_h > 0)
        
        if len(x_h) > 0:
            dist_extremo = np.max(x_h)
            # El 0.96 recorta la "punta" que sobresale de los 60 grados para alinear la grilla
            pixels_por_10_grados = float((dist_extremo * 0.96) / 6.0)
        else:
            dist_extremo = int((ancho - cx) * 0.75)
            pixels_por_10_grados = float(dist_extremo / 6.0)
        ...
        # --- D. ANÁLISIS DE DENSIDAD POR OCTANTES Y MAPA DE CALOR ---
...
st.markdown("""
**Análisis Activo:** Detección Núcleo-Físico (Versión Oro) + Escala Calibrada.
- **Puntos Rojos:** Cuadrados (Fallados).
- **Puntos Verdes:** Círculos (Vistos).
- **Celeste:** Densidad ≥ 70% (10°). **Amarillo:** > 0% (5°).
""")

modo_evaluacion = st.radio("Seleccione el tipo de evaluación:", ["Unilateral (Un solo ojo)", "Bilateral (Ambos ojos)"], key="radio_final_3")
